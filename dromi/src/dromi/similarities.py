...
SimilarityResults = namedtuple("SimilarityResults",
                               ["positional_weights", "percent_identity_mean", "cosine_similarity_mean",
...
class KmersFilling(object):
    ...
    def run(self, weights, hotspots):
        return list(
            map(lambda row_a, row_b, col_a, col_b: self.fill_kmers_array(row_a, row_b, col_a, col_b, weights, hotspots),
                self.rows_idx_a, self.rows_idx_b, self.cols_idx_a, self.cols_idx_b))
...
class MaskedMeanParallel(object):
    ...
    def run(self, pool):
        if self.kmers:
            r = list(
                pool.map(self.masked_mean_loop_kmers, list(zip(zip(*self.iterables), itertools.repeat(self.fixed)))))
            pool.close()
            pool.join()
            return r
        else:
            if self.runtime_option == "ram":
                ...
                r = list(pool.map(self.masked_mean_loop_ondisk,
                                  list(zip(zip(*self.iterables), itertools.repeat(self.fixed)))))
                pool.close()
                pool.join()
                return r
...
def calculate_masked_mean_kmers(iterables_args, fixed_args):  # TODO: Needs to be reviewed or deleted
    ...
    :param fixed_args: Inmmutable/static arguments
                        nkmers: Number of kmers
                        kmers_mask : [N,L,ksize] Mask over kmer values
    """
    ...
    kmers_mask_0 = kmers_mask[
        diag_idx_1]  # (0 and 1 are switched on purpose, it is not an accident), select the mask of the sequences in the batch [batch_size, nkmers,,ksize]
    ...
    hotspots_masked_mean = np.ma.masked_array(hotspots_splits, mask=~hotspots_mask, fill_value=0.).mean(
        1)  # Highlight: In the mask if True means to mask and ignore!!!!
    hotspots_masked_mean = np.ma.masked_array(hotspots_masked_mean, mask=~kmers_mask_0, fill_value=0.).mean(
        2)  # TODO: Should it be mean 3?
...
def calculate_masked_mean(iterables_args, fixed_args):
    ...
    hotspots_masked_mean = np.sum(hotspots_masked_mean.sum(-1), -1) / (hotspots_split.shape[
                                                                           1] - 1)  # .mean(-1) #sum first, then mean accross all sequences (minus 1(itself)), because the other positions are 0
...
def calculate_masked_mean_ondisk(iterables_args, fixed_args):
    ...
    with lock:
        ...
        np.copyto(results_files["positional_weights"][start_idx:end_idx, positional_idx],
                  hotspots_masked_mean.astype(dtype), where=mask)
...
def importance_weight_kmers(hotspots, nkmers, ksize, max_len, positional_mask, overlapping_kmers, batch_size,
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count() - 1, initializer=initializer,
                              initargs=init_args) as pool:
        results = MaskedMeanParallel(args_iterables, args_fixed, kmers=True, runtime_option=runtime_option).run(pool)
    ...
    positional_weights = sum(
        KmersFilling(rows_idx_a, rows_idx_b, cols_idx_a, cols_idx_b).run(positions_weights, hotspots_mean))
    positional_weights /= divisors
    positional_weights = (positional_weights - positional_weights.min()) / (
            positional_weights.max() - positional_weights.min())  # min max scale
...
def importance_weight(hotspots,
    ...
    # Highlight: Masked mean only over neighbouring kmers
    split_size = [int(hotspots.shape[0] / batch_size) if not batch_size > hotspots.shape[0] else 1][0]
    splits_sizes = [split.shape[0] for split in np.array_split(np.arange(n_seqs),
                                                               split_size)]  # Find a more dynamic way of computing this? or inside the loop?
    # Highlight: row dimension storing indexes
    end_split_sizes_cumsum = np.cumsum(splits_sizes)  # [4,3,3] -> [4,7,10]
    start_split_sizes_cumsum = np.insert(end_split_sizes_cumsum[:-1], 0,
                                         0)  # Insert a 0 in the first position, [4,7,10] -> [0,4,7]
...
def process_value_cosine(iterables_args, fixed_args):
    ...
    cosine_sim_pairwise_matrix_ij = np.ma.masked_array(cosine_sim_j, mask=~matrix_mask_ij,
                                                       fill_value=0.)  # [1,L,L] # Highlight: In the mask if True means to mask and ignore!!!!
    ...
    if calculate_kmers:
        kmers_matrix_cosine_diag_ij = kmers_matrix_cosine_ij[:, :, :, :, diag_idx_ksize[0],
                                      diag_idx_ksize[1]]  # does not seem expensive
...
def process_value_pid(iterables_args, fixed_args):
    """Computes similarities metrics (pairwise identity, cosine similarity ... ) among a set of arrays"""
    ...
    if np.ndim(curr_array) == 2:  # Integer encoded #TODO: Delete and force to have dimensions [N,L,1]
        ...
        pairwise_matrix_j = (curr_array[:, None, :, None] == r_j[None, :, None, :]).all((-1)).astype(
            float)  # .all((-2,-1)) #[1,L,L]
    # Highlight: Create masks to ignore the paddings of the sequences
    if calculate_kmers:  # TODO: Eventually move move to function process_value_pid_with_kmers
        kmers_mask_curr_i = curr_mask[:, overlapping_kmers]
        kmers_mask_r_j = r_j_mask[:, overlapping_kmers]
        kmers_mask_ij = (kmers_mask_curr_i[:, None] * kmers_mask_r_j[None, :]).mean(-1)
        kmers_mask_ij[kmers_mask_ij != 1.] = 0.
    ...
    pid_pairwise_matrix_ij = np.ma.masked_array(pairwise_matrix_j, mask=~matrix_mask_ij,
                                                fill_value=0.)  # [1,L,L] #TODO: Discard?
    percent_identity_mean_ij = np.ma.masked_array(pairwise_sim_j, mask=~pid_mask_ij, fill_value=0.).mean(
        -1)  # Highlight: In the mask if True means to mask and ignore!!!!

    # KMERS PERCENT IDENTITY ############
    if calculate_kmers:
        kmers_matrix_pid_diag_ij = kmers_matrix_pid_ij[:, :, :, :, diag_idx_ksize[0],
                                   diag_idx_ksize[1]]  # does not seem expensive
        ...
        kmers_pid_similarity_ij = np.ma.masked_array(kmers_matrix_pid_diag_mean_ij, mask=~kmers_mask_ij,
                                                     fill_value=0.).mean(axis=2)
        del kmers_matrix_pid_diag_mean_ij, kmers_mask_ij, kmers_matrix_pid_ij
    else:
...
def process_value_all(iterables_args, fixed_args):
    """Computes similarities metrics (pairwise identity, cosine similarity ... ) among a set of arrays"""
    ...
    if i == j + shift:  # Highlight: When comparing an array to itself, round to nearest integer the diagonal values, due to precision issues, sometimes it computes 0.999999999 or 1.00000002 instead of 1.
        ...
        :cosine_similarity_mean_ij.shape[1] ** 2:cosine_similarity_mean_ij.shape[1] + 1] = np.rint(
            cosine_similarity_mean_ij.ravel()[
            :cosine_similarity_mean_ij.shape[1] ** 2:cosine_similarity_mean_ij.shape[1] + 1])
    del r_j, r_j_mask, r_j_mask_expanded, rest_splits, curr_array
    del curr_mask_expanded, curr_mask, pid_mask_ij, matrix_mask_ij, cosine_sim_j
    del kmers_mask_ij, kmers_matrix_pid_diag_mean_ij, kmers_matrix_pid_diag_ij, kmers_matrix_pid_ij, kmers_mask_curr_i, kmers_mask_r_j
    del kmers_matrix_cosine_diag_mean_ij, kmers_matrix_cosine_ij
    gc.collect()
    end_i = time.time()
    # print("Time for finishing loop (i vs j) {}".format(str(datetime.timedelta(seconds=end_i - start_i))))
    return cosine_sim_pairwise_matrix_ij, \
...
def process_value_cosine_ondisk(iterables_args,
                                fixed_args):  # TODO: Build common function between process_value and process_value_ondisk
    """Computes similarities metrics (pairwise identity, cosine similarity ... ) among a set of arrays"""
    ...
    if calculate_kmers:
        kmers_matrix_cosine_diag_ij = kmers_matrix_cosine_ij[:, :, :, :, diag_idx_ksize[0],
                                      diag_idx_ksize[1]]  # does not seem to be too expensive
        kmers_matrix_cosine_diag_mean_ij = np.nanmean(kmers_matrix_cosine_diag_ij, axis=4)[:, :, diag_idx_nkmers[0],
                                           diag_idx_nkmers[1]]
        kmers_cosine_similarity_ij = np.ma.masked_array(kmers_matrix_cosine_diag_mean_ij, mask=~kmers_mask_ij,
                                                        fill_value=0.).mean(axis=2)
    ...
    # end_i = time.time()
    # print("Time for finishing loop (i vs j) {}".format(str(datetime.timedelta(seconds=end_i - start_i))))
    del curr_mask, r_j, r_j_mask, curr_mask_expanded, r_j_mask_expanded, matrix_mask_ij
    del cosine_sim_j
    gc.collect()

    with lock:  # NOTE: lock and results_files have been assigned as global variables
        mask = np.ones_like(cosine_similarity_mean_ij).astype(bool)
        np.copyto(results_files["cosine_similarity_mean"][start_store_point:end_store_point,
                  start_store_point_i:end_store_point_i], cosine_similarity_mean_ij.astype(dtype), where=mask)
        np.copyto(results_files["cosine_similarity_mean"][start_store_point_i:end_store_point_i,
                  start_store_point:end_store_point], cosine_similarity_mean_ij.astype(dtype).T,
                  where=mask.T)  # transpose

        mask = np.ones_like(cosine_sim_pairwise_matrix_ij).astype(bool)
        np.copyto(results_files["cosine_sim_pairwise_matrix"][start_store_point:end_store_point,
                  start_store_point_i:end_store_point_i], cosine_sim_pairwise_matrix_ij.astype(dtype), where=mask)
        np.copyto(results_files["cosine_sim_pairwise_matrix"][start_store_point_i:end_store_point_i,
                  start_store_point:end_store_point], cosine_sim_pairwise_matrix_ij.astype(dtype).transpose(1, 0, 2, 3),
                  where=mask.transpose(1, 0, 2, 3))  # transpose

        if calculate_kmers:
            mask = np.ones_like(kmers_cosine_similarity_ij).astype(bool)
            np.copyto(results_files["kmers_cosine_similarity_mean"][start_store_point:end_store_point,
                      start_store_point_i:end_store_point_i], kmers_cosine_similarity_ij.astype(dtype), where=mask)
...
def process_value_pid_ondisk(iterables_args,
                             fixed_args):  # TODO: Build common function between process_value and process_value_ondisk
    """Computes similarities metrics (pairwise identity, cosine similarity ... ) among a set of arrays"""
    ...
    if calculate_kmers:
        kmers_matrix_pid_diag_ij = kmers_matrix_pid_ij[:, :, :, :, diag_idx_ksize[0],
                                   diag_idx_ksize[1]]  # does not seem too expensive
        kmers_matrix_pid_diag_mean_ij = np.mean(kmers_matrix_pid_diag_ij, axis=4)[:, :, diag_idx_nkmers[0],
                                        diag_idx_nkmers[1]]  # if we mask this only it should be fine
        kmers_pid_similarity_ij = np.ma.masked_array(kmers_matrix_pid_diag_mean_ij, mask=~kmers_mask_ij,
                                                     fill_value=0.).mean(axis=2)
    ...
    with lock:  # NOTE: lock and results_files have been assigned as global variables
        ...
        np.copyto(results_files["percent_identity_mean"][start_store_point:end_store_point,
                  start_store_point_i:end_store_point_i], percent_identity_mean_ij.astype(dtype), where=mask)
        np.copyto(results_files["percent_identity_mean"][start_store_point_i:end_store_point_i,
                  start_store_point:end_store_point], percent_identity_mean_ij.astype(dtype).T,
                  where=mask.T)  # transpose
        if calculate_kmers:
            mask = np.ones_like(kmers_pid_similarity_ij).astype(bool)
            np.copyto(results_files["kmers_pid_similarity"][start_store_point:end_store_point,
                      start_store_point_i:end_store_point_i], kmers_pid_similarity_ij.astype(dtype), where=mask)
            np.copyto(results_files["kmers_pid_similarity"][start_store_point_i:end_store_point_i,
                      start_store_point:end_store_point], kmers_pid_similarity_ij.astype(dtype).T,
                      where=mask.T)  # transpose
            del mask
...
def process_value_all_ondisk(iterables_args,
                             fixed_args):  # TODO: Build common function between process_value and process_value_ondisk
    """Computes similarities metrics (pairwise identity, cosine similarity ... ) among a set of arrays"""
    ...
    # KMERS COSINE SIMILARITY ########################
    kmers_matrix_cosine_diag_ij = kmers_matrix_cosine_ij[:, :, :, :, diag_idx_ksize[0],
                                  diag_idx_ksize[1]]  # does not too seem expensive
    kmers_matrix_cosine_diag_mean_ij = np.nanmean(kmers_matrix_cosine_diag_ij, axis=4)[:, :, diag_idx_nkmers[0],
                                       diag_idx_nkmers[1]]
    kmers_cosine_similarity_ij = np.ma.masked_array(kmers_matrix_cosine_diag_mean_ij, mask=~kmers_mask_ij,
                                                    fill_value=0.).mean(axis=2)
...
class SimilarityParallel:
    """Class that wraps the outer and inner for loops for the batched calculation of the cosine similarities"""

    def __init__(self, metric: str, calculate_kmers: bool, iterables: dict, fixed_args: tuple):
        self.calculate_kmers = calculate_kmers
        self.metric = metric
        self.fx = self.select_fx()[self.metric]
        self.fixed = fixed_args
    ...
    def select_fx(self):
        return {"cosine": process_value_cosine,
                "pairwise": process_value_pid,
                "all": process_value_all}

    def inner_loop(self, params):
        """Auxiliary function to SimilarityParallel"""
        iterables, fixed = params
        return self.fx(iterables, fixed_args=fixed)
...
class SimilarityParallelOnDisk:
    ...
    def select_fx(self):
        return {"cosine": process_value_cosine_ondisk,
                "pairwise": process_value_pid_ondisk,
                "all": process_value_all_ondisk}

    def inner_loop(self, params: list):
        """Auxiliary function to SimilarityParallel"""
        iterables, fixed = params
        return self.fx(iterables, fixed_args=fixed)
...
def fill_array(array_fixed, ij, start_i, end_i, start_j, end_j):
    ...
    array_fixed[start_j:end_j, start_i:end_i] = np.swapaxes(ij, 1, 0)  # transpose?
...
def fill_array_map(array_fixed, ij_arrays, starts, ends, starts_j, ends_j):
   ...
   :param int end_j: Indicates the column-wise end position where this batch is allocated
    """
    results = list(
        map(lambda ij, start_i, end_i, start_j, end_j: fill_array(array_fixed, ij, start_i, end_i, start_j, end_j),
            ij_arrays, starts, ends, starts_j, ends_j))
    return results[0]


def calculate_similarities(array: Union[np.ndarray],
                           max_len: int,
                           array_mask: Union[np.ndarray, None],
                           storage_folder: Union[str, None],
                           batch_size: int = 100,
                           ksize: int = 3,
                           neighbours: int = 1,
                           metric: str = ["cosine", "pairwise", "all"],
                           calculate_kmers: bool = False,
                           calculate_positional_weights: bool = False):
    ...
    :param str metric: cosine,  pairwise, all
    :param bool calculate_kmers: calculate also the kmers version of the cosine or pairwise distances
    ...
    if array.size == 0:
        ...
        if array_mask is not None:
            assert array_mask.shape == (
                n_data,
                max_len), "Your dataset mask has dimensions {}, while max_len is {}, please correct that".format(
                array_mask.shape, max_len)
        else:
            warnings.warn(
                "You did not input a mask, therefore I will compute one where everything that is 0 will be masked")
        ...
        assert batch_size <= n_data, "Please select a smaller batch size, current is {}. while dataset size is {}".format(
            batch_size, n_data)
        ...
        split_size = int(array.shape[0] / batch_size) if not batch_size > array.shape[0] else 1
        ...
        # kmers_cosine_similarity_matrix_diag = np.zeros((n_data, n_data,nkmers,nkmers,ksize),dtype=np.uint16)
        ...
        with multiprocessing.Pool(multiprocessing.cpu_count() - 1) as pool:
            ...
            zipped_results = list(zip(*results))
            starts_i, ends_i, starts_j, ends_j = zipped_results[6], zipped_results[7], zipped_results[8], \
                zipped_results[9]

            if metric in ["cosine", "all"]:
                cosine_sim_pairwise_matrix_ij = zipped_results[0]
                #
                # print(cosine_sim_pairwise_matrix_ij[2].shape)
                # print(starts_i)
                # print(ends_i)
                # print(starts_j)
                # print(ends_j)
                #
                # exit()
                cosine_sim_pairwise_matrix = fill_array_map(cosine_sim_pairwise_matrix, cosine_sim_pairwise_matrix_ij,
                                                            starts_i, ends_i, starts_j, ends_j)

                cosine_similarity_mean_ij = zipped_results[2]
                cosine_similarity_mean = fill_array_map(cosine_similarity_mean, cosine_similarity_mean_ij, starts_i,
                                                        ends_i, starts_j, ends_j)
                if calculate_kmers:
                    kmers_cosine_similarity_ij = zipped_results[3]
                    kmers_cosine_similarity_mean = fill_array_map(kmers_cosine_similarity, kmers_cosine_similarity_ij,
                                                                  starts_i, ends_i, starts_j, ends_j)
            ...
            if metric in ["pairwise", "all"]:
                ...
                percent_identity_mean = fill_array_map(percent_identity_mean, percent_identity_mean_ij, starts_i,
                                                       ends_i, starts_j, ends_j)
                if calculate_kmers:
                    kmers_pid_similarity_ij = zipped_results[4]
                    kmers_pid_similarity = fill_array_map(kmers_pid_similarity, kmers_pid_similarity_ij, starts_i,
                                                          ends_i, starts_j, ends_j)

        end = time.time()
        final_time = datetime.timedelta(seconds=end - start)
        print("Overall calculation time {}".format(str(final_time)))
        ...
        if calculate_positional_weights:
            ...
            positional_weights = np.ma.getdata(positional_weights)
            # if calculate_kmers:
        ...
        similarity_results = SimilarityResults(positional_weights=positional_weights,
        ...
        return similarity_results, final_time


def calculate_similarities_ondisk(array: Union[np.ndarray],
                                  max_len: int,
                                  array_mask: Union[np.ndarray, None],
                                  storage_folder: Union[str, None],
                                  batch_size: int = 50,
    ...
    if array.size == 0:
        ...
        results_files = defaultdict()
        if metric in ["pairwise", "all"]:
            percent_identity_mean = np.memmap(f'{storage_folder}/tmp_files/percent_identity_mean.dat', dtype=dtype,
                                              mode='w+', shape=(n_data, n_data))
            results_files["percent_identity_mean"] = percent_identity_mean
            ##pid_pairwise_matrix= np.zeros((n_data, n_data,max_len,max_len))
            if calculate_kmers:
                kmers_pid_similarity = np.memmap(f'{storage_folder}/tmp_files/kmers_pid_similarity.dat', dtype=dtype,
                                                 mode='w+', shape=(n_data, n_data))
                results_files["kmers_pid_similarity"] = kmers_pid_similarity

        if metric in ["cosine", "all"]:
            cosine_similarity_mean = np.memmap(f'{storage_folder}/tmp_files/cosine_similarity_mean.dat', dtype=dtype,
                                               mode='w+', shape=(n_data, n_data))
            cosine_sim_pairwise_matrix = np.memmap(f'{storage_folder}/tmp_files/cosine_sim_pairwise_matrix.dat',
                                                   dtype=dtype, mode='w+', shape=(n_data, n_data, max_len, max_len))
            results_files["cosine_similarity_mean"] = cosine_similarity_mean
            results_files["cosine_sim_pairwise_matrix"] = cosine_sim_pairwise_matrix

            if calculate_kmers:
                kmers_cosine_similarity_mean = np.memmap(f'{storage_folder}/tmp_files/kmers_cosine_similarity_mean.dat',
                                                         dtype=dtype, mode='w+', shape=(n_data, n_data))
        ...
        if calculate_positional_weights:
            ...
            results_files["positional_weights"] = positional_weights
        ...
        #         *args_iterables.values()):  # just put results_files in the args_fixed and comment out the lock to be able to activate this sequential version
        ...
        with multiprocessing.Pool(processes=multiprocessing.cpu_count() - 1, initializer=init_pool_processes,
                                  initargs=(lock, results_files)) as pool:

            results = SimilarityParallelOnDisk(metric, calculate_kmers, args_iterables, args_fixed).outer_loop(pool)
...
def calculate_similarities_cuda_cpu(array: Union[np.ndarray],
                                    max_len: int,
                                    array_mask: Union[np.ndarray, None],
                                    storage_folder: Union[str, None],
                                    batch_size: int = 50,
    ...
    if array.size == 0:
        ...
        if array_mask is not None:
            ...
            warnings.warn(
                "You did not provide a mask, therefore I will compute one where everything that is 0 will be masked")
            array_mask = array.astype(bool)[:, :, 0]

        # assert array_mask.dtype == np.bool, "Please define the mask as a boolean, where True indicates amino acid and False padding"
        assert batch_size <= n_data, "Please select a smaller batch size, current is {}. while dataset size is {}".format(
            batch_size, n_data)
        assert array.ndim == 3, "Please encode your sequences with ndim = 3 ,such that if you have a 1 dimensional vector per sequence they will be encoded as [N,max_len,1]"
        array = array.astype('float16')
